...
/**
 *
 *  Exports a tab delimitted file of the content of the name_usage table.
...
public class ChecklistBankExporter {
    ...
    private String identifierSql = "SELECT identifier FROM tmp_identifiers WHERE lexical_group_fk =? and name_fk = ? ORDER BY id";
    ...
    //private FileOutputStream idFileOut = null;
    ...
    //                    log.debug("After getLSID " + (System.currentTimeMillis() - startTime) + " ms");
                        ...
                        String line = id +"\t"+parentFk +"\t"+lsid + "\t"+canId+"\t" + scientificName + "\t" + replaceNull(rs.getString("authorship"))+"\t" + portalRank +"\t" +rank;
                        if(denormalise){
        ...
        private String getLSID(Integer nameFK,  Integer lexGrpFK) throws Exception{
            ...
            List<String> identifiers = dTemplate.queryForList(identifierSql, new Object[]{lexGrpFK, nameFK}, String.class);
            ...
            if(identifiers!= null && identifiers.size() > 0){
                String lsid = (String)identifiers.get(0);
//  NC: This is commented out until we need to support alternative lsid mapping file
...
//                    idFileOut.write((id + "\t" + identifier + "\n").getBytes());
//                }
                
                return replaceNull(lsid);
