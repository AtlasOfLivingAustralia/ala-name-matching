...
object ProcessOutliersUsingFile {
        ...
        for (keyVariable <- keyOnVariables) {
            ...
            if (!erroneousGuids.isEmpty) {
                ...
                for (erroneousGuid <- erroneousGuids) {
                    ...
                    //println("GUID: " + tiGuid)
                    mutator.writeColumn("outliers", tiGuid, mutator.newColumn("outlier-" + keyVariable,
    ...
    def main(args:Array[String]){
      ...
      val filePath = args(0)
        //val linesToIgnore = args(1).toInt

      val fr = new BufferedReader(new FileReader(filePath))
        //for (i <- 0 until linesToIgnore) fr.readLine

      val columnHeaders =  fr.readLine.split("\t")
      ...
      while (currentLine != null){
         ...
         if (columns.size>5){
            val taxonConceptID = columns(1)

            if(currentTaxonID != taxonConceptID){
               taxaProcessed += 1
               print(taxaProcessed + ", processing: " + currentTaxonID+", elements buffered: " + currentListBufferSize)
               //process this batch
