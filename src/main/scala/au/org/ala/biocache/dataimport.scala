...
trait DataLoader {
   ...
   */
    ...
    def getDataResourceDetailsAsMap(resourceUid:String) : Map[String, String] = {
      val json = Source.fromURL(Config.registryURL + "/ws/dataResource/" + resourceUid + ".json").getLines.mkString
      JSON.parseFull(json).get.asInstanceOf[Map[String, String]]
    }

    def getDataProviderDetailsAsMap(uid:String) : Map[String, String] = {
      val json = Source.fromURL(Config.registryURL + "/ws/dataProvider/" + uid + ".json").getLines.mkString
      JSON.parseFull(json).get.asInstanceOf[Map[String, String]]
    }

    def getInstitutionDetailsAsMap(uid:String) : Map[String, String] = {
      val json = Source.fromURL(Config.registryURL + "/ws/institution/" + uid + ".json").getLines.mkString
      JSON.parseFull(json).get.asInstanceOf[Map[String, String]]
    }
    ...
    def updateLastChecked(resourceUid:String,dataCurrency:Option[Date]=None) :Boolean ={
        try {
          ...
          if(Config.allowCollectoryUpdates == "true"){
            ...
            val responseCode = Http.postData(Config.registryURL + "/ws/dataResource/" +resourceUid,data).header("content-type", "application/json").responseCode
