...
object VerbatimLatLongParser {
  ...
  val verbatimPattern = """([0-9]{1,3})(°[ ]*)([0-9]{1,2})?('[ ]*)?([0-9]{1,2}.?[0-9]{0,})?("[ ]*)?(s|south|n|north|w|west|e|east)""".r
  val negativePattern = "(s|south|w|west)".r
  
  /**
   * Parses the verbatim latutide or longitude
   * Formats handled:
   * 30° 01' S
   * 153° 12' E
   * 145° 44' 55.85" E
  ...
  def parse(stringValue:String) : Option[Float]={
    try{
        ...
        normalised match{
            case verbatimPattern(degree, dsign, minute, msign, second, ssign, direction) => convertToDecimal(degree, minute, second, direction)
            case _ =>None
        }
