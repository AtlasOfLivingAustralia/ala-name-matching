...
class MiscTest extends ConfigFunSuite {
  ...
  test("missing basis of record") {
    ...
    expectResult(0) {
      qas.find(_.code == 20001).get.qaStatus
    }
  }
  ...
  test("badly formed basis of record") {
    ...
    val qas = (new BasisOfRecordProcessor).process("test", raw, processed)
    expectResult(0) {
      qas.find(_.code == 20002).get.qaStatus
    }
  }
  ...
  test("unrecognised type status") {
    ...
    expectResult(0) {
      qas.find(_.code == 20004).get.qaStatus
    }
  }
  ...
  test("invalid collection date") {
    ...
    expectResult(1) {
      qas.find(_.code == 30007).get.qaStatus
    }
  ...
  test("Default DwC Values") {
    ...
    expectResult("HumanObservation") {
      processed.occurrence.basisOfRecord
    }
  ...
  test("interactions") {
    ...
    expectResult("Infects") {
      processed.occurrence.interactions(0)
    }
  }
  test("modified") {
    val raw = new FullRecord
    ...
    expectResult("2004-08-17") {
      processed.occurrence.modified
    }
  }
  ...
  test("establishment means") {
    ...
    expectResult("not cultivated; not native") {
      processed.occurrence.establishmentMeans
    }
    raw.occurrence.establishmentMeans = "not cultivated; missing from vocab"
    mp.process("test", raw, processed)
    expectResult("not cultivated") {
      processed.occurrence.establishmentMeans
    }
